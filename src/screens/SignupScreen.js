import React, { useState } from 'react';
    ...
    export default function SignupScreen({ navigation }) {
      ...
      const [role, setRole] = useState('job_seeker'); // Ensure this is "job_seeker" with underscore

      const radioButtons = [
        { id: 'job_seeker', label: 'Job Seeker', value: 'job_seeker' }, // Ensure this matches
      ...
      const handleSignup = async () => {
        ...
        if (error) {
          ...
          const insertData = {
            ...
            role,
            ...(data.user && { auth_users_id: data.user.id }),
          };
          console.log('Role being inserted:', role); // Debug role
          console.log('Full insert data:', insertData); // Debug full data
          const { error: insertError } = await supabase.from('users').insert(insertData);

          if (insertError) {
            console.log('Insert error:', insertError.message); // Debug
